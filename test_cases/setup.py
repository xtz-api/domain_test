...
def setup_domain_addShoppingList(volc_domnain_client, data):
    if data["comment"] == "购物车已满，加购未被注册域名":
        ...
        domain_info = db_check.domain_get_shopingList_by_domain(data["input"]["test_cases"])
        ...
        if domain_info != ():
            ...
            assert_that(_).is_equal_to(200)
        assert_that(db_check.domain_get_shopingList_by_domain(data["input"]["test_cases"])).is_empty()
        while len(db_check.domain_get_shopingList_db()) ==100:
        ...
        assert_that(db_check.domain_get_shopingList_by_domain(data["input"]["test_cases"])).is_not_empty()