...
def get_domain_list(volc_domnain_client, num):
    ...
    domain_list = []
    time_s = 283824000
    data = {"test_domain": "sdgfuwhdiuehfu.cn",
            "period": 1,
            "zone": ".cn",
            "renew_price": "5000"}
    status = []
    ...
    for domain_data in resp["Result"]["domain_info_list"]:
        if num != 10:
            if domain_data["status"] in [1, 2, 4] and domain_data["expired_time"] - int(time.time()) <= time_s:
                if not zhmodel.search(domain_data["test_domain"][:len(domain_data["zone"])]):
                    if len(domain_list) == num:
                        break
                    data["test_domain"] = domain_data["test_domain"]
...
def get_domain_data(volc_domnain_client, **kwargs):
    ...
    for j in range(0, 20):
        ...
        domain = resp["Result"]["domain_info_list"][i]["test_domain"]
